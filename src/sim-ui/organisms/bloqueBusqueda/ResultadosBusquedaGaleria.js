...
import { GaleriaResultados } from "../extraCard/GaleriaTarjeta";
...
const ResultadosBusquedaGaleria = (props) => {
    ...
    useEffect(() => {
        ...
        resultados.map(r => {
            const records_ = r._source.document.records.filter(
                (d) => d.support === 'Galería fotográfica',
            )
            ...
            array_imagenes = [...array_imagenes, ...records_]
        })

        setImagenes(array_imagenes)
    }, [resultados])
    ...
    return (
        <>
            {resultados !== undefined && (
                <>
                    {(!loading || !props.loading) && imagenes.length > 0 ? (
                        <>
...
export default ResultadosBusquedaGaleria;
